...
# Time Machine is an optional system surface, not a user-cache candidate. The
# GUI can skip its snapshot/failed-backup probes without skipping Mole's own
# developer and orphan modules (which are part of the cleanup contract).
mole_skip_time_machine_enabled() {
    [[ "${MOLE_SKIP_TIME_MACHINE:-0}" == "1" ]]
}
...
perform_cleanup() {
    ...
    local skip_time_machine=false
    if mole_skip_time_machine_enabled; then
        skip_time_machine=true
        debug_log "Skipping Time Machine snapshot and failed-backup probes for cleanup preview"
    fi
    ...
    run_clean_sections() {
        if [[ -n "$EXTERNAL_VOLUME_TARGET" ]]; then
            ...
            if [[ "$SYSTEM_CLEAN" == "true" ]]; then
                ...
                # Local snapshot enumeration is part of the optional Time
                # Machine surface.  It is not a cleanable candidate and can
                # block on tmutil, so leave it to the full standalone sweep.
                if [[ "$skip_time_machine" != "true" ]]; then
            ...
            # Mole is the authoritative developer-cache inventory. The GUI
            # no longer runs a second broad app_dev_scan walker.
            ...
            # ===== 10. App leftovers =====
            # Mole owns the broad orphan sweep; the GUI supplement only adds
            # exact cache/log paths backed by a trashed Bundle ID.
            ...
            # ===== 13. Time Machine =====
            # Failed-backup discovery can traverse external/backup volumes and
            # is not part of the GUI's safe user-cache inventory.  Keep it in
            # the full standalone command, not the latency-sensitive preview.
            if [[ "$skip_time_machine" != "true" ]]; then
            ...
            # ===== 14. Large files =====
            if [[ "${MOLE_SKIP_LARGE_FILE_HINTS:-0}" != "1" ]]; then
            ...
            if [[ "${MOLE_SKIP_PROJECT_HINTS:-0}" != "1" ]]; then
