...
func createInsightEntries() []dirEntry {
	...
	// Large developer and AI data is analysis-only. These paths are surfaced
	// as first-class overview rows so users do not have to discover multi-GB
	// runtimes, package stores or model/session data by drilling through Home.
	reviewPaths := []struct {
		name string
		path string
	}{
		{"nvm Node Versions", filepath.Join(home, ".nvm", "versions", "node")},
		{"npm Cache", filepath.Join(home, ".npm")},
		{"pnpm Store", filepath.Join(home, "Library", "pnpm", "store")},
		{"Yarn Cache", filepath.Join(home, ".yarn", "cache")},
		{"Maven Repository", filepath.Join(home, ".m2", "repository")},
		{"Cargo Registry", filepath.Join(home, ".cargo", "registry")},
		{"Go Module Cache", filepath.Join(home, "go", "pkg", "mod")},
		{"Ollama Models", filepath.Join(home, ".ollama", "models")},
		{"HuggingFace Models", filepath.Join(home, ".cache", "huggingface")},
		{"LM Studio Models", filepath.Join(home, ".cache", "lm-studio", "models")},
		{"PyTorch Models", filepath.Join(home, ".cache", "torch")},
		{"Claude Sessions", filepath.Join(home, ".claude", "projects")},
		{"Codex Sessions", filepath.Join(home, ".codex", "sessions")},
		{"Codex App Data", filepath.Join(home, "Library", "Application Support", "Codex")},
	}
	insightPaths = append(insightPaths, reviewPaths...)
	if matches, err := filepath.Glob(filepath.Join(home, "Library", "Group Containers", "*dev.orbstack", "data")); err == nil {
		for _, match := range matches {
			if info, statErr := os.Stat(match); statErr == nil && info.IsDir() {
				insightPaths = append(insightPaths, struct {
					name string
					path string
				}{"OrbStack Data", match})
				break
