...
# The GUI cleanup preview has its own path-based developer-cache inventory and
# never executes `brew autoremove`; probing Homebrew's dependency solver here
# only adds latency (and can block on a broken/isolated brew installation).
# Keep the upstream preview for the CLI and allow the app orchestrator to opt
# out explicitly without changing standalone Mole behavior.
skip_brew_autoremove_preview() {
    [[ "${MOLE_SKIP_HOMEBREW_AUTOREMOVE:-0}" == "1" ]]
}
...
clean_homebrew() {
    ...
    if [[ "${DRY_RUN:-false}" == "true" ]]; then
        ...
        if is_path_whitelisted "$HOME/Library/Caches/Homebrew"; then
            ...
            note_activity
            if skip_brew_autoremove_preview; then
                debug_log "Homebrew autoremove preview skipped by orchestrator"
            else
