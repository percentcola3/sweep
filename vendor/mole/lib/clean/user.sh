...
_clean_mail_downloads() {
    ...
    local -a mail_dirs=("$HOME/Library/Mail Downloads")
    if [[ "${SIMPLEMOLE_SKIP_PROTECTED_APP_DATA:-0}" != "1" ]]; then
        mail_dirs+=("$HOME/Library/Containers/com.apple.mail/Data/Library/Mail Downloads")
    fi
    local count=0
    local cleaned_kb=0
...
clean_app_caches() {
    ...
    # Sandboxed app caches. Never expand these paths during background scans
    # unless Full Disk Access has already been positively confirmed.
    if [[ "${SIMPLEMOLE_SKIP_PROTECTED_APP_DATA:-0}" != "1" ]]; then
    ...
    [[ "${SIMPLEMOLE_SKIP_PROTECTED_APP_DATA:-0}" == "1" ]] && return 0
...
clean_application_support_logs() {
    ...
    if [[ "${SIMPLEMOLE_SKIP_PROTECTED_APP_DATA:-0}" != "1" ]]; then
        known_group_containers=("group.com.apple.contentdelivery")
    fi
...
check_large_file_candidates() {
    ...
    if [[ "$docker_reported" != "true" &&
        "${SIMPLEMOLE_SKIP_PROTECTED_APP_DATA:-0}" != "1" ]]; then
    ...
    _report_large_review_dir "OrbStack data" "$HOME/OrbStack"
