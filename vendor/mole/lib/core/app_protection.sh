...
find_app_files() {
    ...
    local include_protected_app_data=true
    if [[ "${SIMPLEMOLE_SKIP_PROTECTED_APP_DATA:-0}" == "1" ]]; then
        include_protected_app_data=false
    fi
    ...
    for p in "${user_patterns[@]+"${user_patterns[@]}"}"; do
        ...
        if [[ "$include_protected_app_data" != "true" ]]; then
    ...
    if [[ "$bundle_id_valid" == "true" ]]; then
        ...
        if [[ "$include_protected_app_data" == "true" &&
            -d "$HOME/Library/Group Containers" ]]; then
            discovery_scan_rc=0
            _mole_uninstall_materialize_find0 "$discovery_scan_file" \
        ...
        local -a derived_bundle_roots=("$HOME/Library/Application Support/FileProvider")
        if [[ "$include_protected_app_data" == "true" ]]; then
            derived_bundle_roots+=(
                "$HOME/Library/Application Scripts"
                "$HOME/Library/Containers"
            )
        fi
    ...
    if [[ "$bundle_id_valid" == "true" && -n "$app_path" ]]; then
        ...
        while IFS= read -r embedded_id; do
            ...
            local -a embedded_candidates=(
                ...
                "$HOME/Library/WebKit/$embedded_id"
            )
            if [[ "$include_protected_app_data" == "true" ]]; then
                embedded_candidates+=(
            ...
            for embedded_candidate in "${embedded_candidates[@]}"; do
    ...
    if [[ "$bundle_id" == "com.raycast.macos" ]]; then
        ...
        local raycast_dirs=("$HOME/Library/Application Support")
        if [[ "$include_protected_app_data" == "true" ]]; then
            raycast_dirs+=(
                "$HOME/Library/Application Scripts"
                "$HOME/Library/Containers"
