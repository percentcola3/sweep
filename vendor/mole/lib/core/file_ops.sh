...
mole_delete() {
    ...
    # Optional caller-owned policy check at the final mutation edge. The hook
    # is inert unless a trusted bridge installs a shell function explicitly.
    local final_guard="${MOLE_DELETE_FINAL_GUARD:-}"
    if [[ -n "$final_guard" ]]; then
        if [[ ! "$final_guard" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]] ||
            ! declare -F "$final_guard" > /dev/null 2>&1 ||
            ! "$final_guard" "$path"; then
            _mole_delete_log "$mode" "$size_kb" "final-guard" "$path"
            log_operation "${MOLE_CURRENT_COMMAND:-clean}" "SKIPPED" "$path" "final guard"
            return 1
        fi
    fi
