...
run_with_timeout() {
    ...
    if [[ -n "${MO_TIMEOUT_PERL_BIN:-}" ]]; then
        ...
        "$MO_TIMEOUT_PERL_BIN" -e '
            ...
            # Most filesystem probes and identity checks finish in a few
            # milliseconds. Poll them promptly at startup so the timeout
            # wrapper does not add a fixed 100ms tax to every short command.
            # After the brief startup window, return to the original 100ms
            # cadence to keep long-running commands inexpensive to monitor.
            my $started_at = time();
            my $fast_poll_deadline = $started_at + 0.2;
